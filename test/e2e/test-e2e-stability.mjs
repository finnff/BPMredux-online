...
/**
 * All 9 test tracks: 3 genres × 3 patterns (original, increasing, decreasing)
 */
const TEST_TRACKS = [
  // Techno (~136 BPM)
...
function calculateJitter(readings) {
  ...
  const stable = readings.filter(r => r.time > 10);
  if (stable.length < 2) return 0;
  const mean = stable.reduce((s, r) => s + r.bpm, 0) / stable.length;
  const variance = stable.reduce((s, r) => s + (r.bpm - mean) ** 2, 0) / stable.length;
  return Math.sqrt(variance);
}

/**
 * Time until first reading with confidence > 0.1.
 */
function calculateResponseTime(readings) {
  const first = readings.find(r => r.confidence > 0.1);
  return first ? first.time : null;
}

/**
 * Compute accuracy metrics against a librosa ground truth timeline.
 *
 * @param {Array} readings - [{time, bpm, confidence}, ...]
